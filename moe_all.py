...
class MoE_receive_gate(nn.Module):
    ...
    def __init__(self, input_size, output_size, num_experts, hidden_size, model=MLP,noisy_gating=True, k=4,all_mode=False,special_for_gate=256):
        ...
        self.k = k
        # instantiate experts
        self.w_gate = nn.Parameter(torch.zeros(special_for_gate, num_experts), requires_grad=True)
        self.w_noise = nn.Parameter(torch.zeros(special_for_gate, num_experts), requires_grad=True)
        self.experts = nn.ModuleList([model(self.input_size, self.output_size, self.hidden_size) for i in range(self.num_experts)])
        self.softplus = nn.Softplus()
        self.softmax = nn.Softmax(1)
        self.register_buffer("mean", torch.tensor([0.0]))
